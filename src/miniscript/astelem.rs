    ...
    fn from_tree(top: &expression::Tree) -> Result<Terminal<Pk>, Error> {
        ...
        for ch in frag_wrap.chars().rev() {
            match ch {
                ...
                'n' => unwrapped = Terminal::ZeroNotEqual(Arc::new(Miniscript::from_ast(unwrapped)?)),
                't' => unwrapped = Terminal::AndV(Arc::new(Miniscript::from_ast(unwrapped)?), Arc::new(Miniscript::from_ast(Terminal::True)?)),
                'u' => unwrapped = Terminal::OrI(Arc::new(Miniscript::from_ast(unwrapped)?), Arc::new(Miniscript::from_ast(Terminal::False)?)),
                'l' => unwrapped = Terminal::OrI(Arc::new(Miniscript::from_ast(Terminal::False)?), Arc::new(Miniscript::from_ast(unwrapped)?)),
