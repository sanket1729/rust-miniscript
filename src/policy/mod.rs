...
impl<Pk: MiniscriptKey> Liftable<Pk> for Terminal<Pk> {
    fn into_lift(&self) -> Semantic<Pk> {
        match *self {
            ...
            | Terminal::ZeroNotEqual(ref sub) => sub.node.into_lift(),
...
impl<Pk: MiniscriptKey> Liftable<Pk> for Descriptor<Pk> {
    fn into_lift(&self) -> Semantic<Pk> {
        match *self {
            ...
            | Descriptor::ShWsh(ref d) => d.node.into_lift(),
            Descriptor::Pk(ref p)
    ...
    fn into_lift(&self) -> Semantic<Pk> {
        match *self {
            ...
            Concrete::And(ref subs) => {
                Semantic::And(subs.iter().map(Liftable::into_lift).collect())
            }
            Concrete::Or(ref subs) => {
                Semantic::Or(subs.iter().map(|&(_, ref sub)| sub.into_lift()).collect())
            }
            Concrete::Threshold(k, ref subs) => {
                Semantic::Threshold(k, subs.iter().map(Liftable::into_lift).collect())
            }
        }
        .normalized()
    }
